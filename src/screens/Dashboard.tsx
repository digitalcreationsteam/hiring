...
export default function Dashboard() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      /* LISTS */
      console.log("Setting work experience:", res?.data?.workExperience?.length || 0);
      ...
      const mappedCertifications = (res?.data?.certifications || []).map((c: any) => ({
      ...
      const skillsList = (res?.skills?.list || []).map((s: string) => ({ name: s }));
      ...
      if (userUniversity) {
        ...
        console.warn("No university found in education list — skipping leaderboard fetch");
      }
      ...
      if (demoId) {
        try {
          ...
          console.log("Weekly case study attempts:", weeklyAttempts, "percentage:", pct);
  ...
  const handleAssessmentClick = () => {
    console.log("handleAssessmentClick called, completed:", isAssessmentCompleted);
  ...
  const skillProgress = useMemo(() => {
    const progress = hireability.skill.max > 0
  ...
  const experienceProgress = useMemo(() => {
    const progress = hireability.experience.max > 0
  ...
  const circleOffset = useMemo(() => {
    ...
    const offset = CIRCUMFERENCE - (CIRCUMFERENCE * hireability.totalScore) / MAX_SCORE;
    console.log("circleOffset calculated:", offset, "totalScore:", hireability.totalScore);
    return offset;
            ...
            <div className="flex w-full flex-col gap-8 lg:max-w-[800px]">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  console.log(`Rendering rank card ${i}:`, rank.label, rank.val);
                  return (
                    <Card
                      key={i}
