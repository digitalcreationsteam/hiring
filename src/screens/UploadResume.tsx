  ...
  const uploadResume = async () => {
    ...
    if (!file && existingResume) {
      try {
        ...
        setUploading(true);
        console.log("📤 Using existing resume, fetching navigation status...");
        const navigation = await fetchStatusWithRetry(token);
    ...
    try {
      ...
      // Double-check localStorage before upload
      console.log("🔍 Checking localStorage before upload:");
      ...
      // Upload resume - your API will automatically add headers from localStorage
      console.log("🚀 Starting upload to:", URL_PATH.uploadResume);

      // Make the API call
      const uploadResult = await API("POST", URL_PATH.uploadResume, formData);

      console.log("✅ Upload successful:", uploadResult);
      toast.success("Resume uploaded successfully");

      // Wait for backend to update status
      console.log("⏳ Waiting for backend to process resume...");
      const navigation = await fetchStatusWithRetry(token);
          ...
          >
            ...
            <div className="mb-8">
              ...
              >
                <button
                  ...
                  style={{ color: colors.neutral[600] }}
                >
                  <FeatherArrowLeft style={{ width: "16", height: "16" }} />
                  Back
                </button>
              </motion.div>
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: colors.neutral[600] }}>
                  Upload your most recent resume to help us understand your
                  skills
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <motion.div
                  role="button"
                  tabIndex={0}
                  aria-label="Upload resume. Click or press Enter to browse files. Or drag and drop a file here."
                  className="w-full flex flex-col items-center justify-center gap-4 rounded-xl border-2 border-dashed bg-white px-6 py-8 cursor-pointer transition-all"
                  style={{
                    borderColor: focused ? colors.primary : colors.neutral[300],
            ...
            >
              <motion.button
                onClick={uploadResume}
                disabled={(!file && !existingResume) || uploading}
                className="w-full h-11 font-medium rounded-lg text-white flex items-center justify-center gap-2 transition hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
                style={{
                  backgroundColor:
                    (!file && !existingResume) || uploading
                      ? colors.neutral[300]
                ...
                whileHover={
                  (!file && !existingResume) || uploading ? {} : { scale: 1.02 }
                }
                whileTap={
                  (!file && !existingResume) || uploading ? {} : { scale: 0.98 }
                }
              >
                {uploading ? (
                  <>
